...
package object threading {
  ...
  val generator: Observable[Int] = Observable.from(1 to maxNumber).debug("Observable.onDoNext")
  ...
  def debug(message: String, value: Int, indent: Int): Unit = {
    printf("%30s %-14s %" + (indent * 4) + "s%n", "[" + Thread.currentThread().getName + "]", message, value+" numThreads:"+Thread.activeCount())
  }
}
