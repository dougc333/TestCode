...
if __name__ == '__main__':
    ...
    random.seed(1234)
    print 'sampling 200 from swedish_train_corpus()'
    subdata = random.sample(data, 200)
    
    try:
        print 'calling TransitionParser'
        #tp = TransitionParser(Transition, FeatureExtractor)
        #tp.train(subdata)
        #tp.save('swedish.model')
        #print 'swedish model saved'
        
        testdata = dataset.get_swedish_test_corpus().parsed_sents()
        print 'loading swedish test data'
        tp = TransitionParser.load('badfeatures.model')
        print 'loaded badfeatures.model'
