...
def main(fraglen, pdb):
	...
	print("pdb_id:",pdb_id," fraglen:",fraglen)
	fragfile = '../starter_data/{}_{}mers.frag'.format(pdb_id, fraglen)	
	print("fragfile:",fragfile)
	frag = open(fragfile, 'r')
	data = open('{}_{}mers.rmsd'.format(pdb_id, fraglen), 'w')
	AVG = []
	...
	for i in range(1, int(size)+1):
		...
		print("fragset:",fragset)
		fragset.frames(i, frames)
		movemap = MoveMap()
		movemap.set_bb(True)
		for frame in frames:
			print("frame:",frame)
			print("frame.nr_frags()+1:",frame.nr_frags()+1)
